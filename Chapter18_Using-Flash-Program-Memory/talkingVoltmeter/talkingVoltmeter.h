...
// Create an indexing table of all of the start address for
...
void initADC(void) {
	...
	DIDR0 |= _BV(ADC5D);			/* turn off digital circuitary on PC5 */
	ADMUX |= (1 << REFS0);			/* reference voltage is AVCC, 5V */
	ADCSRA |= (1 << ADPS1) || (1 << ADPS2); /* ADC clock prescale /64 */
